...
void Keyboard::HandleScanCode(unsigned char scanCode, int expand)
{
	...
	/* 新增：处理方向键上 (Up Arrow) */
    case 0x48: 
        if (expand == 0xE0) // 确认是扩展键（方向键）而不是小键盘 '8'
        {
    ...
    /* 新增：处理方向键下 (Down Arrow) */
    case 0x50:
        if (expand == 0xE0) // 确认是扩展键（方向键）而不是小键盘 '2'
        {
            TTy* pTTy = Kernel::Instance().GetDeviceManager().GetCharDevice(DeviceManager::TTYDEV).m_TTy;
            if ( NULL != pTTy )
            {
    ...
    case 0x49:
        if (expand == 0xE0)
        {
            if (Mode & (M_LALT | M_RALT))
            {
                // Alt + PageUp: Scroll Debug Window
                if (Diagnose::m_ViewStartRow > 0)
                {
                    Diagnose::m_ViewStartRow--;
                    Diagnose::RefreshScreen();
                }
            }
            else
            {
                // Normal PageUp: Scroll CRT Window
    ...
    case 0x51:
        if (expand == 0xE0)
        {
             if (Mode & (M_LALT | M_RALT))
            {
                // Alt + PageDown: Scroll Debug Window
                if (Diagnose::m_ViewStartRow + Diagnose::ROWS <= Diagnose::m_BufferHead)
                {
                    Diagnose::m_ViewStartRow++;
                    Diagnose::RefreshScreen();
                }
            ...
            {
                // Normal PageDown: Scroll CRT Window
