...
SystemCallTableEntry SystemCall::m_SystemEntranceTable[SYSTEM_CALL_NUM] = 
{
	...
	{ 2, &Sys_Ssig	},				/* 48 = sig	*/
	{ 0, &Sys_Getppid },			/* 49 = getppid	*/
	{ 1, &Sys_Getpids },			/* 50 = getpids	*/
	{ 1, &Sys_Getproc },			/* 51 = getproc	*/
...
int SystemCall::Sys_Getpids()
{
	...
	int *ppid =(int *) u.u_arg[0]; //u_arg存放系统调用的参数，这里取ppid的指针
	*ppid=u.u_procp->p_ppid;

	u.u_ar0[User::EAX] = u.u_procp->p_pid; //pid依旧放eax
...
{
    User& u = Kernel::Instance().GetUser();
    Process* proc = u.u_procp;
    
    // 定义与用户态一致的结构体
    struct proc_info {
        unsigned long code_vaddr;
        unsigned int code_size;
        unsigned long data_vaddr;
        unsigned int data_size;
        unsigned int stack_size;
        unsigned long code_paddr;
        unsigned long swap_paddr;
        int pid;
        int ppid;
        int uid;
        int state;
    ...
    // 获取用户态缓冲区指针
    proc_info* pinfo = (proc_info*)u.u_arg[0];
    
    /* 填充虚拟空间信息 */
    pinfo->code_vaddr = u.u_MemoryDescriptor.m_TextStartAddress;
    pinfo->code_size = u.u_MemoryDescriptor.m_TextSize;
    pinfo->data_vaddr = u.u_MemoryDescriptor.m_DataStartAddress;
    pinfo->data_size = u.u_MemoryDescriptor.m_DataSize;
    pinfo->stack_size = u.u_MemoryDescriptor.m_StackSize;
    
    /* 填充物理空间信息 */
    // 代码段物理地址(如果有共享正文段)
    pinfo->code_paddr = proc->p_textp->x_caddr;
    // 可交换部分物理地址
    pinfo->swap_paddr = proc->p_addr;
    
    /* 填充进程状态信息 */
    pinfo->pid = proc->p_pid;
    pinfo->ppid = proc->p_ppid;
    pinfo->uid = proc->p_uid;
    pinfo->state = proc->p_stat;
    pinfo->pri = proc->p_pri;
